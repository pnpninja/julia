...
template<typename T>
static inline typename std::enable_if<sizeof(T) == 1, T>::type
jl_atomic_load(volatile T *obj)
{
    return (T)_InterlockedOr8((volatile char*)obj, char(0));
}
template<typename T>
static inline typename std::enable_if<sizeof(T) == 2, T>::type
jl_atomic_load(volatile T *obj)
{
    return (T)_InterlockedOr16((volatile short*)obj, short(0));
}
template<typename T>
static inline typename std::enable_if<sizeof(T) == 4, T>::type
jl_atomic_load(volatile T *obj)
{
    return (T)_InterlockedOr((volatile LONG*)obj, LONG(0));
}
template<typename T>
static inline typename std::enable_if<sizeof(T) == 8, T>::type
jl_atomic_load(volatile T *obj)
{
    return (T)_InterlockedOr64((volatile __int64*)obj, __int64(0));
}
#  define jl_atomic_load_acquire(obj) jl_atomic_load(obj)
#else
